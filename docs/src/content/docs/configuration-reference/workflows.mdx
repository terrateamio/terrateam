...
:::note
Be careful when modifying `lock_policy`. Any value other than `strict` is considered less safe.
use alternatives only if necessary.  
For example, `merge` may be appropriate for development branches where locking is less critical, but is not recommended for production.
:::
