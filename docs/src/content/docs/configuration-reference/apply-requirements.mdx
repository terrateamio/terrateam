...
In this context, "noop" (no operation) means there are no Terraform or OpenTofu changes in the pull request. If `create_completed_apply_check_on_noop` is set to `true`, Terrateam will still create and mark the apply status check as complete, even when there is nothing to apply.
</Aside>
