...
- **Enhanced Security**: Credentials are automatically rotated and expire after a short period
- **No Credential Storage**: Eliminates the need to store long-lived access keys
- **Simplified Management**: Reduces operational overhead of rotating and managing static credentials
- **Audit Trail**: Better tracking of which workflows accessed which resources
- **Fine-grained Control**: Easily implement role-based access control per workflow
</Card>
...
All sensitive values (like role ARNs or service account credentials) should be stored as [GitHub Secrets](https://docs.github.com/en/actions/security-guides/encrypted-secrets) and referenced using the `${SECRET_NAME}` syntax. This ensures credentials are encrypted and only accessible during workflow execution.
:::
  ...
  - tag_query: "dir:aws"
    plan:
      - type: oidc  # Authentication step
        provider: aws
        role_arn: ${AWS_ROLE_ARN}  # Stored as a GitHub Secret
...
2. **Configure Environment Variables**
   Add the credentials to your Terrateam configuration:
   ```yaml
   hooks:
     all:
       pre:
...
3. **Set Required Permissions**
   Ensure your script and the credentials it generates follow least privilege principles:
   - Script should only have permissions to assume specific roles
   - Generated credentials should have minimal required permissions
   - Use different permission sets for plan vs. apply operations
</Steps>
