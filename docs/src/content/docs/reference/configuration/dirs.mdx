...
The `dirs` configuration can be used to describe which [tag queries](/advanced-workflows/tags), workspaces, and [when modified](/reference/configuration/when-modified) rules apply to a directory in your repository.
...
| create_if_missing | boolean | Automatically create the directory before running any workflow steps if it does not exist. Default is false. |
...
### Create If Missing
The `create_if_missing` key specifies whether Terrateam should automatically create the directory before running any workflow steps (plan, apply, init, etc.) if it does not already exist. This is useful when directories are dynamically generated, such as when using the [config builder](/workflows/advanced/terragrunt-config-builder) to register directories produced by tools like `terragrunt stack generate`.
...
When used with the config builder, set `create_if_missing: true` on dir entries that reference directories generated at runtime:
