...
These endpoints provide system-level functionality including administrative operations, server configuration, OAuth setup, and asynchronous task management.

## Administrative Endpoints

### Get Drift Detection Results

Retrieve drift detection results across all repositories (admin only).

**Endpoint:** `GET /api/v1/admin/drifts`

**Responses:**

- **200**: Success - Returns drift detection results
...
```bash
curl -X GET \
  https://app.terrateam.io/api/v1/admin/drifts \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```

:::note[Admin Access Required]
This endpoint requires administrator privileges. Regular users will receive a 403 Forbidden response.
:::

**Use Cases:**
- Monitor drift across all repositories from a central dashboard
- Generate drift reports for compliance
- Identify infrastructure that has diverged from code
- Track drift detection frequency and results
...
- [Drift Detection](/governance/drift-detection) - Learn about drift detection configuration
- [Self-Hosted Setup](/quickstart/self-hosted/) - Configure self-hosted Terrateam instances
- [GitHub Integration](/quickstart/cloud/first-plan-apply) - Setting up GitHub authentication
