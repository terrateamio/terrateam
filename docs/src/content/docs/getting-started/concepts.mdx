...
For example, consider a repository named `repo` with two directories, `dir1` and `dir2`, and no extra workspace configuration. This repository would have two Dirspaces:
...
Terrateam offers two configurable automated operations:

- **Auto-Plan**: (Enabled by default) Runs `terraform plan` on new or updated pull requests. Configure in `.terrateam/config.yml`:
  ```yaml
