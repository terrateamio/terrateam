...
- **Label values:** `force="true"` for `atlantis apply -force` / `terrateam apply-force` operations, `force="false"` for regular applies (including autoapply and stack auto-apply)
