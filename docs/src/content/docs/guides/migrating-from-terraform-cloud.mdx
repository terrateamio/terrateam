...
4. Migrate the State  
     - Temporarily configure local state (remove/comment out the remote backend).  
     - Place the downloaded state file in your working directory (`terraform.tfstate`).  
     - Re-enable the S3 backend config.  
