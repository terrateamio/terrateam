...
Before getting started with Terrateam, make sure you have:

- **A GitHub repository** with Terraform code
- **GitHub Actions** enabled in your repository
- **Administrator access** to install the Terrateam GitHub app
- **Cloud provider credentials** (optional for initial setup, required for production use)

## Demo Repository

Want to follow along with a working example? You can fork our [Terrateam example repository](https://github.com/terrateamio/terrateam-example) to practice with pre-configured Terraform code.

<LinkCard
  title="Terrateam Example Repository"
  href="https://github.com/terrateamio/terrateam-example"
  icon="github"
/>
...
1. **Install the Terrateam GitHub Application**
   ...
   - $149/mo unlimited with 14-day free trial (no credit card required)
...
3. **Configure Cloud Provider Authentication (Optional for this guide)**
   ...
   For real infrastructure deployments, you'll need to set up authentication with your cloud provider.
...
The `null_resource` in Terraform lets you run provisioners without creating any actual infrastructure. It's perfect for testing workflows since it doesn't modify any real resources.
:::
