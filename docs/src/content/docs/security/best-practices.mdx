...
Use [GitHub Environments](/integrations/github-environments) in combination with [access control](/reference/configuration/access-control) to isolate sensitive resources and secrets:
   ```yaml
   workflows:
...
3. Enable [drift detection](/reference/configuration/drift) to identify and track changes made to your infrastructure outside of Terraform:
   ```yaml
   hooks:
     plan:
       post:
...
4. Configure [apply requirements](/reference/configuration/apply-requirements) to enforce code reviews and approval processes:
   ```yaml
   apply_requirements:
     checks:
       - tag_query: 'dir:production'
