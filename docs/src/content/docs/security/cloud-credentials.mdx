...
Configure OIDC for different cloud providers, such as AWS, Azure, and GCP, within the same workflow.
```yaml
workflows:
...
This example demonstrates how to configure OIDC for AWS, Azure, and GCP within the same workflow, enabling multi-cloud deployments.
