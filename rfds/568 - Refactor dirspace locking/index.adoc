...
Terraform/Tofu state locking, which ensure serialized access to a state during a
specific Terraform/Tofu operation.  Dirspace locking ensures that a change to a
set of dirspaces which are in-progress prevent any overlapping changes from
...
`DS1`, Terrateam should not consider these pull requests locking each other.
This is because the destination branches are different.
...
. The user comments on the pull request `terrateam unlock` (dirspace `DS1` is no
unlocked).
...
This definition of locks also makes it difficult to query the database for which
dirspaces are locked and by whom, as that is an even more expensive query.
...
this achieves our goal of reducing the complexity of querying locks.
...
as $$
    ...
    locked as (
        ...
        inner join all_dirspaces_for_pull_request as adspr
            on gpr.repository = adspr.repository
               and gpr.pull_number = adspr.pull_number
    ...
    deleted as (
        ...
        where gprm.repository_id = $1
              and gprm.pull_number = $2
              and gprm.core_id = dsprl.pull_request
              and not exists (select repository from locked)
    ...
    select i.path, i.workspace from inserted i;
$$;
----
...
Migrating to this will be implemented over the following steps:

. Release the new tables and triggers to maintain those tables.
. Release migration bring any existing locks into the new system.
. Release change that queries the new table instead of the old method.

=== Migrating existing locks into new system.
