...
This will operate independent of the chosen strategy. Given stem:[N] dirspaces, each generating an output stem:[O_i], stem:[i \in \{1..n\}], and a limit stem:[L] for a comment on particular VCS platform. We must proceed with the following steps:
...
NOTE: For Github, a comment is limited to 65 kb.
