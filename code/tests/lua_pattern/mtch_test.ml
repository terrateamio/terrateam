...
let compare_mtch res = function
  | None ->
    res = None
  | Some m ->
    res = (Some (Lua_pattern.Match.range m))
...
let compare_captures captures = function
  ...
  | Some m ->
    let mtch_captures =
      List.map
        ~f:(fun c ->
          (Lua_pattern.Capture.start c, Lua_pattern.Capture.stop c))
...
let create_test (str, pat, res, captures) =
  Oth.test
    ~name:(Printf.sprintf "mtch %s %s" str pat)
    (test_mtch str pat res captures)
