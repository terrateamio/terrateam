...
let rebuilder = { Bs.Rebuilder.run = (fun st _k _v task fetcher -> task st fetcher) }

let test_const =
  Oth.test ~name:"const" (fun _ ->
      let a1 : int Hmap.key = Hmap.Key.create () in
      let st = Bs.St.create () in
      let tasks_map = Hmap.empty |> Hmap.add (coerce a1) (fun _ _ -> 10) in
...
let test_dynamic =
  Oth.test ~name:"dynamic" (fun _ ->
      ...
      let tasks_map =
        Hmap.empty
        |> Hmap.add (coerce a1) (fun _ _ -> 10)
        |> Hmap.add (coerce b1) (fun _ { Bs.Fetcher.fetch } -> fetch a1 + 1)
      in
      let tasks = { Bs.Tasks.get = (fun _ k -> Hmap.find (coerce k) tasks_map) } in
...
let test_dynamic2 =
  Oth.test ~name:"dynamic2" (fun _ ->
      ...
      let tasks_map =
        ...
        |> Hmap.add (coerce a1) (fun _ _ -> 10)
        |> Hmap.add (coerce b1) (fun _ { Bs.Fetcher.fetch } -> fetch a1 + 1)
        |> Hmap.add (coerce b2) (fun _ { Bs.Fetcher.fetch } -> fetch a1 + fetch b1)
      in
      let tasks = { Bs.Tasks.get = (fun _ k -> Hmap.find (coerce k) tasks_map) } in
