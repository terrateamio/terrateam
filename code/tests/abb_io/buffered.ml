...
let read_line =
  Oth_abb.test
    ...
    (fun () ->
       ...
       >>= fun read_ret ->
       assert (read_ret = Ok "foo");
       Buffered.read_line r
       >>= fun read_ret ->
       assert (read_ret = Ok "bar");
       Buffered.read_line r
       >>= fun read_ret ->
       assert (read_ret = Error (`Unexpected End_of_file));
       Fut.return ())

let close_gives_read_eof =
  Oth_abb.test
    ~desc:"Closing writer gives EOF on reader"
    ~name:"Close gives read EOF"
    (fun _ ->
