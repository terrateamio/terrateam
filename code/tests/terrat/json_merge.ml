let test_bool =
  Oth.test ~name:"Test Bool" (fun _ ->
      ...
      assert (Terrat_json.merge ~base override = Ok (`Bool false));
      let base = `Bool false in
      let override = `Bool true in
      assert (Terrat_json.merge ~base override = Ok (`Bool true));
      let base = `Null in
      let override = `Bool true in
      assert (Terrat_json.merge ~base override = Ok (`Bool true));
      let base = `Bool true in
      let override = `Null in
      assert (Terrat_json.merge ~base override = Ok `Null))

let test_int =
  Oth.test ~name:"Test Integer" (fun _ ->
      let base = `Int 1 in
      let override = `Int 2 in
      assert (Terrat_json.merge ~base override = Ok (`Int 2));
      let base = `Null in
      let override = `Int 1 in
      assert (Terrat_json.merge ~base override = Ok (`Int 1));
      let base = `Int 1 in
      let override = `Null in
      assert (Terrat_json.merge ~base override = Ok `Null);
...
let test_type_mismatch_err =
  Oth.test ~name:"Test type mismatch err" (fun _ ->
      ...
      assert (
        Terrat_json.merge ~base override = Error (`Type_mismatch_err (None, `Int 1, `String "foo")));
      let base = `Assoc [ ("k", `Int 1) ] in
      let override = `Assoc [ ("k", `String "foo") ] in
      assert (
        Terrat_json.merge ~base override
        = Error (`Type_mismatch_err (Some "k", `Int 1, `String "foo")));
      let base = `Assoc [ ("j", `Assoc [ ("k", `Int 1) ]) ] in
      let override = `Assoc [ ("j", `Assoc [ ("k", `String "foo") ]) ] in
      assert (
        Terrat_json.merge ~base override
        = Error (`Type_mismatch_err (Some "j.k", `Int 1, `String "foo"))))
