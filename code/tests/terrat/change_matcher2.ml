let ctx = Terrat_change_match2.Ctx.make ~dest_branch:"main" ~branch:"test" ()
      ...
      let dirs =
        CCResult.get_exn
          (Terrat_change_match2.synthesize_config
             ...
             repo_config)
      in
      let changes = Terrat_change_match2.match_diff_list dirs diff in
      ...
      let dirs =
        CCResult.get_exn
          (Terrat_change_match2.synthesize_config
             ...
             repo_config)
      in
      let changes = CCList.flatten (Terrat_change_match2.match_diff_list dirs diff) in
      assert (CCList.length changes = 1);
...
let test_dir_match =
  Oth.test ~name:"Test dir match" (fun _ ->
      ...
      let changes =
        CCList.filter
          (Terrat_change_match2.match_tag_query
             ~tag_query:(CCResult.get_exn (Terrat_tag_query.of_string "dir:ec2")))
          (CCList.flatten (Terrat_change_match2.match_diff_list dirs diff))
      in
      assert (CCList.length changes = 1))
      ...
      let changes =
        CCList.filter
          (Terrat_change_match2.match_tag_query
             ~tag_query:(CCResult.get_exn (Terrat_tag_query.of_string "dir:ec2")))
          (CCList.flat_map
             CCFun.(Terrat_change_match2.of_dirspace dirs %> CCOption.to_list)
             dirspaces)
      in
      assert (CCList.length changes = 1))
...
let test_dir_config_iam =
  Oth.test ~name:"Test Dir Config IAM" (fun _ ->
      ...
      CCList.iter
        (fun {
               Terrat_change_match2.Dirspace_config.dirspace = { Terrat_dirspace.dir; _ };
               when_modified;
               _;
             } ->
...
let test_index_basic =
  Oth.test ~name:"Test basic index" (fun _ ->
      let module Idx = Terrat_change_match2.Index in
      let index = Idx.make ~symlinks:[] [ ("tf", Idx.Dep.[ Module "../modules/foo" ]) ] in
      let repo_config = Terrat_base_repo_config_v1.make () in
      let file_list = [ "modules/foo/main.tf"; "tf/main.tf" ] in
      let dirs =
        CCResult.get_exn (Terrat_change_match2.synthesize_config ~ctx ~index ~file_list repo_config)
      in
      let diff = Terrat_change.Diff.[ Add { filename = "modules/foo/main.tf" } ] in
      let changes = CCList.flatten (Terrat_change_match2.match_diff_list dirs diff) in
      assert (CCList.length changes = 1);
      let dirspace = (CCList.hd changes).Terrat_change_match2.Dirspace_config.dirspace in
      assert (
                            ...
                            ( "default",
                              R.Dirs.Workspace.make
                                ~when_modified:
                                  (R.When_modified.make
                                     ~depends_on:
                                       (CCResult.get_exn (Terrat_tag_query.of_string "dir:base"))
                                     ())
                                () );
                          ])
                     () );
