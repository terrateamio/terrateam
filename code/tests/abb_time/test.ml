...
let wall_diff_prop =
  Oth.test
    ~desc:"Wall clock diff property"
    ~name:"Wall diff property"
    (fun _ ->
...
let mono_diff_prop =
  Oth.test
    ~desc:"Monotonic diff property"
    ~name:"Monotonic diff property"
    (fun _ ->
...
let () =
  ...
  Oth.(
    run
      (parallel [ loop 10 wall_diff_prop
                ; loop 10 mono_diff_prop
                ]))
