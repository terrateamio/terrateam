...
module Group_by (G : GROUP_BY) : sig
  (** Given a list, group each element in the list by an associated key and return a list mapping
      keys to the values. This guarantees that the list in each group is in the same order as they
      were in the original list, however there are no guarantees around the groups. *)
