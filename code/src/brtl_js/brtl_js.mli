...
(** Given a [base], merge [on_true] in when the bool signal is [true] and [on_false] when it is
    [false] *)
val tri_merge :
  'a list -> on_true:'a list -> on_false:'a list -> bool React.signal -> 'a list React.signal

(** Merge the second list with the first list when the signal is [true] or just the first list when
    [false]. [flip] applies [not] to the signal's value *)
