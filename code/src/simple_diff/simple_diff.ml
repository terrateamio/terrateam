    ...
    let rec outer trace = function
      ...
      | d -> (
          let rec inner k =
            if k <= d then (
              let x =
                if k = -d || (k <> d && v.(max + k - 1) < v.(max + k + 1)) then v.(max + k + 1)
                else v.(max + k - 1) + 1
              in
              let y = x - k in
              let rec nested x y =
              ...
              v.(max + k) <- x;
              if x >= old_len && y >= revised_len then `Stop d else inner (k + 2))
            else `Cont
          in
  ...
  let backtrack old revised trace =
    ...
    let rec outer track x y d = function
      ...
      | v :: vs ->
          let k = x - y in
          let prev_k =
            if k = -d || (k <> d && v.(max + k - 1) < v.(max + k + 1)) then k + 1 else k - 1
          in
          ...
          let rec inner track x y =
            if x > prev_x && y > prev_y then inner ((x - 1, y - 1, x, y) :: track) (x - 1) (y - 1)
            else if d > 0 then ((prev_x, prev_y, x, y) :: track, prev_x, prev_y)
            else (track, prev_x, prev_y)
          in
