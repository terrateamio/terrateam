<script lang="ts">
	...
	// Register diff language for highlight.js immediately (not in onMount)
	...
	function generateFilename(): string {
		...
		const baseName =
			parts.length > 0 ? parts.join('_').replace(/[^a-zA-Z0-9\-_]/g, '_') : 'terraform_output';
...
{#if !showContent}
	...
	<div
		class="border border-yellow-200 dark:border-yellow-700 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4"
	>
...
{:else}
	...
	<div class="space-y-3">
		...
		<div
			class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 px-3 py-2 rounded"
		>
		...
		{#if isPlan && highlightedContent}
			...
			<pre
				class="plan-hljs text-xs bg-gray-900 dark:bg-gray-950 p-3 rounded overflow-x-auto whitespace-pre-wrap font-mono {showFullContent
					? ''
					: 'max-h-96'}"
			><code>{@html showFullContent ? highlightedContent : highlightedPreview}</code></pre>
		{:else}
			<!-- Plain text output (non-plan or highlighting failed) -->
			<pre
				class="text-xs bg-gray-900 dark:bg-gray-950 text-gray-100 dark:text-gray-200 p-3 rounded overflow-x-auto whitespace-pre-wrap font-mono {showFullContent
					? ''
					: 'max-h-96'}"
			>
