...
export function areTrialBannersEnabled(): boolean {
  if (typeof window !== 'undefined' && window.terrateamConfig?.trial_banners) {
    return window.terrateamConfig.trial_banners === 'true';
  }
  // Default to enabled for SaaS mode
  return isSaasBillingMode();
}

/**
 * Calculate days remaining until a trial ends
 * Returns null if trial_ends_at is not set or already expired
 */
export function getTrialDaysRemaining(trialEndsAt: string | undefined): number | null {
