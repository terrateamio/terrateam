<script lang="ts">
  ...
  // Email state
  let installationEmail: string = '';
  let isEmailSaving = false;
  let emailSaveError: string | null = null;
  ...
  // Load email only when installation ID changes (not on every render)
  $: if ($selectedInstallation && $selectedInstallation.id !== lastInstallationId) {
    lastInstallationId = $selectedInstallation.id;
    installationEmail = $selectedInstallation.email || '';
  }
  ...
  async function handleSaveEmail(): Promise<void> {
    if (!$selectedInstallation) {
      emailSaveError = 'No installation selected';
      return;
    }

    if (!installationEmail || !installationEmail.includes('@')) {
    ...
    try {
      await api.updateInstallationEmail($selectedInstallation.id, installationEmail, currentProvider);

      // Update the selected installation in the store
      if ($selectedInstallation) {
        $selectedInstallation.email = installationEmail;
      ...
      emailSaveError = err instanceof Error ? err.message : 'Failed to save email address';
    } finally {
      isEmailSaving = false;
    }
  }
      ...
      <div class="max-w-4xl mx-auto">
        ...
        <div class="card-bg rounded-lg shadow border mb-6">
          <div class="px-4 py-6 md:px-6 md:py-8">
            ...
            <!-- Email Address -->
            <div class="mb-8">
              <label for="installation-email" class="block text-sm font-medium text-brand-secondary mb-3">
                {capitalizedOrganization} Email Address
              </label>
              <p class="text-sm text-brand-secondary mb-4">
                Provide an email address for important notifications about this {terminology.organization.toLowerCase()}.
                {#if !$selectedInstallation}
                  Please select {articleForOrganization} {terminology.organization.toLowerCase()} first.
                {/if}
              </p>
              <div class="flex items-start space-x-3">
                <div class="flex-1 max-w-md">
                  <input
                    ...
                    type="email"
                    bind:value={installationEmail}
                    disabled={!$selectedInstallation || isEmailSaving}
                  ...
                  />
                  {#if emailSaveError}
                    <p class="mt-2 text-sm text-red-600 dark:text-red-400">{emailSaveError}</p>
                  {/if}
                  {#if emailSaveSuccess}
                    <p class="mt-2 text-sm text-green-600 dark:text-green-400">Email saved successfully!</p>
                  {/if}
                </div>
                <button
                  on:click={handleSaveEmail}
                  disabled={!$selectedInstallation || isEmailSaving || !installationEmail}
                  class="inline-flex items-center px-4 py-2 font-medium text-sm rounded-lg transition-colors bg-blue-600 hover:bg-blue-700 text-white disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                >
                  {#if isEmailSaving}
