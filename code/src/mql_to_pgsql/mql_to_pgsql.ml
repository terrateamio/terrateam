...
let mk_page_compare cond page col expr =
  ...
  | Some `Null -> Ok (cond expr Mql_ast.Null)
  | Some (`String s) -> Ok (cond expr (Mql_ast.String s))
  | Some (`Float fl) -> Ok (cond expr (Mql_ast.Float fl))
  | Some (`Int i) -> Ok (cond expr (Mql_ast.Int i))
  | Some ((`Tuple _ | `Bool _ | `Intlit _ | `Variant _ | `Assoc _ | `List _) as v) ->
      Error (`Page_invalid_type_err v)

let mk_where_cond col_alias_to_expr page order_by =
  let module A = Mql_ast in
  let open CCResult.Infix in
  let cond =
    match page.Page.dir with
    | Page.Affirm -> (
        fun dir e_query e_page ->
          match dir with
          | None | Some A.Asc -> A.Gte (e_query, e_page)
          | Some A.Desc -> A.Lte (e_query, e_page))
    | Page.Negate -> (
        fun dir e_query e_page ->
          match dir with
          | None | Some A.Asc -> A.Lte (e_query, e_page)
          | Some A.Desc -> A.Gte (e_query, e_page))
  in
  CCResult.map_l
    (function
      | (A.Identifier col as e), dir -> (
          match String_map.find_opt col col_alias_to_expr with
          | Some (A.Count A.Star) -> Ok None
          | Some expr -> mk_page_compare (cond dir) page col expr >>= fun c -> Ok (Some c)
          | None -> mk_page_compare (cond dir) page col e >>= fun c -> Ok (Some c))
      | (A.Field_select (A.Identifier _table, col) as e), dir ->
          mk_page_compare (cond dir) page col e >>= fun c -> Ok (Some c)
      | A.Count A.Star, _ -> Ok None
      | _ -> assert false)
    order_by
  >>= fun cs ->
  match CCList.filter_map CCFun.id cs with
  | [] -> Ok None
  | [ cond ] -> Ok (Some cond)
  | c :: cs -> Ok (Some (CCListLabels.fold_left ~init:c ~f:(fun acc c -> A.And (acc, c)) cs))

let mk_having_cond col_alias_to_expr expr_to_col_alias page order_by =
  let module A = Mql_ast in
  let open CCResult.Infix in
  let cond =
    match page.Page.dir with
  ...
  CCResult.map_l
    (function
      | A.Identifier col, dir -> (
          match String_map.find_opt col col_alias_to_expr with
          | Some (A.Count A.Star as expr) ->
              mk_page_compare (cond dir) page col expr >>= fun c -> Ok (Some c)
          | Some _ | None -> Ok None)
      | A.Field_select _, _ -> Ok None
      | (A.Count A.Star as expr), dir -> (
          match Col_expr_map.find_opt Col_expr.Count_star expr_to_col_alias with
          | Some alias -> mk_page_compare (cond dir) page alias expr >>= fun c -> Ok (Some c)
          | None -> mk_page_compare (cond dir) page "count" expr >>= fun c -> Ok (Some c))
      | _ -> assert false)
    order_by
  >>= fun cs ->
  match CCList.filter_map CCFun.id cs with
...
let rec apply_page page t =
  ...
  | A.Query
      ...
      let expr_to_col_alias = expr_to_col_alias select_list in
      mk_where_cond col_alias_to_expr page order_by
      >>= fun where_cond ->
      let where =
        match where_cond with
        | Some where_cond ->
            Some
              (CCOption.map_or ~default:where_cond (fun where -> A.And (where_cond, where)) where)
        | None -> where
      in
      mk_having_cond col_alias_to_expr expr_to_col_alias page order_by
      >>= fun having_cond ->
      match (group_by, having_cond) with
      | None, Some _ -> Error `Missing_group_by_err
      | Some _, Some having_cond ->
          let having =
            Some
              (CCOption.map_or
                 ~default:having_cond
                 (fun having -> A.And (having_cond, having))
                 having)
          in
          let order_by = apply_page_dir order_by page.Page.dir in
          Ok
            (A.Query
               { body = A.Select { select with A.where; having }; order_by = Some order_by; limit })
      | _, _ ->
          let order_by = apply_page_dir order_by page.Page.dir in
