...
(* The resulting topology maps a dirspace to all those dirspace configs which
          ...
          ( working_dirspace,
            {
              ...
              stack_config =
                { S.rules = { Rules.modified_by; plan_after; apply_after; auto_apply = _ }; _ };
              _;
            } )
        ->
        let all_stack_deps = modified_by @ plan_after @ apply_after in
        CCListLabels.fold_left
          ~f:(fun acc (dirspace, { Dirspace_config.tags; stack_name; _ }) ->
            ...
            then Dirspace_map.add_to_list dirspace working_dirspace acc
            else acc)
          ~init:acc
          dirspaces)
