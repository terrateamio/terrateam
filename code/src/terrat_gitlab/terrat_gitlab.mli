...
val create : Terrat_config.Gitlab.t -> Openapic_abb.Authorization.t -> Openapic_abb.t

val with_client :
  Terrat_config.Gitlab.t ->
  Openapic_abb.Authorization.t ->
  (Openapic_abb.t -> 'a Abb.Future.t) ->
  'a Abb.Future.t
...
val call :
  ...
  Openapic_abb.t ->
  'a Openapi.Request.t ->
  ('a Openapi.Response.t, [> Openapic_abb.call_err ]) result Abb_scheduler_kqueue.Future.t
...
val fetch_pull_request_files :
  ...
  pull_number:int ->
  Openapic_abb.t ->
  (Githubc2_components.Diff_entry.t list, [> Openapic_abb.call_err | `Error ]) result Abb.Future.t
...
module Oauth : sig
  ...
  val authorize :
    config:Terrat_config.Gitlab.t -> string -> (Response.t, [> authorize_err ]) result Abb.Future.t

  val refresh :
    config:Terrat_config.Gitlab.t -> string -> (Response.t, [> refresh_err ]) result Abb.Future.t
end
