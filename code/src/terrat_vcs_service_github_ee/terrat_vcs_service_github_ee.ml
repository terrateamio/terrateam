module Provider : Terrat_vcs_provider2_github.S = struct
  ...
  module Db = struct
    ...
    let store_flow_state ~request_id work_manifest_id state = raise (Failure "nyi")
    ...
    let query_conflicting_work_manifests_in_repo ~request_id db pull_request dirspaces =
      raise (Failure "nyi")
