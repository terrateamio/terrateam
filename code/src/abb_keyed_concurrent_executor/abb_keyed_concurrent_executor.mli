(** A keyed concurrent executor allows the concurrent execution of work that is serialized based on
    its derived keys. This allows a piece of work to serialize across multiple dimensions. For
    example, consider a system with users. When renaming a user, the operation to perform the rename
    could have the new username and the old username as keys, guaranteeing no other rename
    operations interfer.
