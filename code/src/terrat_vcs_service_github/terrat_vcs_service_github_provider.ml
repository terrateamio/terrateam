...
let target_repo = function
  | Terrat_vcs_provider2.Target.Pr pr -> Terrat_vcs_api_github.Pull_request.repo pr
  | Terrat_vcs_provider2.Target.Drift { repo; _ } | Terrat_vcs_provider2.Target.Adhoc { repo; _ } ->
      repo

let target_pull_number_opt = function
  | Terrat_vcs_provider2.Target.Pr pr -> Some (Terrat_vcs_api_github.Pull_request.id pr)
  | Terrat_vcs_provider2.Target.Drift _ | Terrat_vcs_provider2.Target.Adhoc _ -> None

let target_repo_id target = Terrat_vcs_api_github.Repo.id (target_repo target)
...
let target_id_and_is_pr = function
  | Terrat_vcs_provider2.Target.Pr pr ->
      (CCInt.to_string (Terrat_vcs_api_github.Pull_request.id pr), true)
                                   ...
                                   CCList.filter_map
                                     (function
                                       | `Run_kind ->
                                           Some
                                             ( "run-kind",
                                               `String (target_run_kind work_manifest.Wm.target) )
                                       | `Run_type -> (
                                       ...
                                       | `Pr_title -> (
                                           ...
                                           | Terrat_vcs_provider2.Target.Pr pr -> (
                                               ...
                                               | None -> None)
                                           | Terrat_vcs_provider2.Target.Drift _
                                           | Terrat_vcs_provider2.Target.Adhoc _ -> None)
                                       | `Pr_number -> (
                                           match work_manifest.Wm.target with
                                           | Terrat_vcs_provider2.Target.Pr pr ->
                                               Some
