...
module Pull_request = struct
  ...
  let of_api api =
    {
      base_branch_name = Api.Pull_request.base_branch_name api;
      base_ref = Api.Pull_request.base_ref api;
      branch_name = Api.Pull_request.branch_name api;
      branch_ref = Api.Pull_request.branch_ref api;
      id = Api.Pull_request.id api;
      repo = Api.Pull_request.repo api;
      state = Api.Pull_request.state api;
      title = Api.Pull_request.title api;
      user = Api.Pull_request.user api;
    }
end
                ...
                (fun () ->
                  Pgsql_io.Prepared_stmt.fetch
                    ...
                    ~f:(fun
                      ...
                      ->
                      {
                        Pull_request.base_branch_name = Api.Ref.of_string base_branch_name;
                        ...
                        state =
                          ...
                          | _ -> assert false);
                        title;
                        user = CCOption.map Api.User.make user;
                      })
                    work_manifest_id)
              >>= function
              | [] -> assert false
  ...
  let store_pull_request ~request_id db pull_request =
    ...
    Metrics.Psql_query_time.time (Metrics.psql_query_time "insert_pull_request") (fun () ->
        Pgsql_io.Prepared_stmt.execute
          ...
          (CCOption.map Api.User.to_string @@ Pr.user pull_request))
    >>= function
    | Ok () -> Abb.Future.return (Ok ())
    | Error (#Pgsql_io.err as err) ->
        Prmths.Counter.inc_one Metrics.pgsql_errors_total;
        ...
        Pgsql_io.Prepared_stmt.fetch
          ...
          ~f:(fun
            ...
            ( Terrat_change.Dirspace.{ dir; workspace },
              {
                Pull_request.base_branch_name = Api.Ref.of_string base_branch;
