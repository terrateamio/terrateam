let src = Logs.Src.create "vcs_service_github_ep_mql"
...
(* The MQL table allow-list.
   ...
   GitHub-specific table behind the scenes (e.g. [github_work_manifests]). *)
let schema =
  Mql_to_pgsql.Schema.(
    make
      [
...
let execute_query config storage user installation_id tz query =
  let open Abbs_future_combinators.Infix_result_monad in
  let statement_timeout =
    Terrat_config.statement_timeout @@ Terrat_vcs_service_github_provider.Api.Config.config config
  in
  Pgsql_pool.with_conn storage ~f:(fun db ->
      Pgsql_io.tx db ~f:(fun () ->
          ...
          Pgsql_io.Prepared_stmt.fetch
            db
            (Sql.select_page (Mql.Ast.to_string @@ Mql_to_pgsql.query query))
            ~f:CCFun.id
            user
...
(* Run an MQL query end to end: parse the page cursor, build the query, execute
   it, trim the result set, and decide its pagination links. *)
let run_query config storage user installation_id q tz page =
  let open Abbs_future_combinators.Infix_result_monad in
  ...
  build_query q page
  >>= fun (query, limit) ->
  execute_query config storage user installation_id tz query
  >>= fun rows ->
  (* If there are more rows than requested, it means there are further pages. *)
...
(* Respond with a 400 carrying an [Error_response]. *)
...
(* Log an MQL error [err] with [pp], then respond with a 400 [Error_response]. *)
...
module Make (S : S with type Account_id.t = int) = struct
  ...
  let run config storage installation_id q tz page =
    Brtl_ep.run_result_json ~f:(fun ctx ->
        ...
        >>= fun () ->
        let open Abb.Future.Infix_monad in
        run_query config storage (Terrat_user.id user) installation_id q tz page
        >>= respond ctx
