-- MQL query wrapper (GitHub installation scoped).
--
-- SECURITY: each CTE below shadows a real table with a copy filtered to a
-- single GitHub installation that the calling user is a member of (the user
...
-- filters on $installation_id and joins github_user_installations2 on
-- $user_id, mirroring select_work_manifests_page.sql.
with
accessible_repos as (
  select
    gir.id as id,
