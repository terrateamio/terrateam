...
let rec merge' ~null_is_identity ~list_append ~base override =
  ...
  | `List b, `List o ->
      if list_append then Ok (`List (CCList.append b o)) else Ok (`List (CCList.append o b))
  | (`Assoc b as base), `Assoc o ->
      ...
      CCResult.fold_l
        (fun acc (k, v) ->
          ...
          | `Null -> Ok ((k, v) :: acc)
          | v' -> (
              match merge' ~null_is_identity ~list_append ~base:v' v with
              | Ok v -> Ok ((k, v) :: acc)
      ...
      in
      Ok (`Assoc override)
  | v, `Null when null_is_identity -> Ok v
  | _, `Null -> Ok `Null
  | `Null, v -> Ok v
...
let merge ?(null_is_identity = false) ?(list_append = false) ~base override =
  (merge' ~null_is_identity ~list_append ~base override
