module Verifier = struct
  ...
  let mac_verify algo hp signature =
    Cryptokit.hash_string algo hp = signature
...
module Header = struct
  ...
  let create ?(rest = []) ?(typ = "JWT") alg =
    ("typ", typ)::("alg", alg)::rest
  ...
  let of_string str =
    ...
    CCOpt.wrap Yojson.Basic.from_string str
    >>= fun json ->
    of_json json
end
