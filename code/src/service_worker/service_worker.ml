...
module Background_fetch_registration = struct
  ...
  let abort (t : t) =
    ...
    Abb_js.Future.unsafe_of_promise t##abort >>| Js.to_bool

  let match_all (t : t) =
    let open Abb_js.Future.Infix_monad in
    Abb_js.Future.unsafe_of_promise t##matchAll
...
module Registration = struct
  ...
  let background_fetch (t : t) id requests options =
    Abb_js.Future.unsafe_of_promise
  ...
  let background_fetch_ids (t : t) =
    let open Abb_js.Future.Infix_monad in
    Abb_js.Future.unsafe_of_promise t##.backgroundFetch##getIds
    >>| fun ids -> ids |> Js.to_array |> Array.to_list |> List.map Js.to_string

  let background_fetch_get (t : t) id =
    Abb_js.Future.unsafe_of_promise (t##.backgroundFetch##get (Js.string id))
end
...
module Service_worker_container = struct
  ...
  let ready (t : t) = Abb_js.Future.unsafe_of_promise t##.ready
end
...
module Cache = struct
  ...
  let put (t : t) request response = Abb_js.Future.unsafe_of_promise (t##put request response)

  let delete (t : t) url =
    let open Abb_js.Future.Infix_monad in
    Abb_js.Future.unsafe_of_promise (t##delete (Js.string url)) >>| Js.to_bool
end
...
module Cache_storage = struct
  ...
  let keys (t : t) =
    let open Abb_js.Future.Infix_monad in
    Abb_js.Future.unsafe_of_promise t##keys
    >>| fun keys -> CCList.map Js.to_string (Array.to_list (Js.to_array keys))

  let delete (t : t) n = Abb_js.Future.unsafe_of_promise (t##delete (Js.string n))
end
...
module Global_scope = struct
  ...
  module Fetch_event = struct
    ...
    let respond_with (t : t) fut =
      Abb_js.Future.run fut;
      t##respondWith (Abb_js.Future.unsafe_to_promise fut)
  end
  ...
  module Clients = struct
    ...
    let claim (t : t) = Abb_js.Future.unsafe_of_promise t##claim
  end
