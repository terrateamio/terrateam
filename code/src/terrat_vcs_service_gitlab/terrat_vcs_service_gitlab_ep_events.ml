...
module Make (P : Terrat_vcs_provider2_gitlab.S) = struct
  ...
  let dispatch_event config storage installation_id ctx =
    ...
    | E.Push_event
        {
          Pe.project = { Pr.id = repo_id; name; namespace; default_branch; _ };
          ref_;
          user_username;
          _;
        }
      when ref_ = "refs/heads/" ^ default_branch ->
        ...
        let repo = P.Api.Repo.make ~id:repo_id ~name ~owner:namespace () in
    ...
    | E.Merge_request_comment_event
        {
          Mrce.project = { Pr.id = repo_id; name; namespace; _ };
          object_attributes =
    ...
    | E.Merge_request_event
        {
          Mre.project = { Pr.id = repo_id; name; namespace; _ };
          user = { User.username; _ };
