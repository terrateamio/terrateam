module Oauth = Terrat_vcs_service_user.Oauth

let src = Logs.Src.create "vcs_service_gitlab_ep_groups"

module Logs = (val Logs.src_log src : Logs.LOG)

let rec get' config storage user =
...
let get config storage =
  Brtl_ep.run_result ~f:(fun ctx ->
      ...
      | Ok groups ->
          let body =
            groups |> Terrat_api_gitlab_groups.List.Responses.OK.to_yojson |> Yojson.Safe.to_string
          in
