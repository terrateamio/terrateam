-- MQL query wrapper (GitLab installation scoped, admin variant).
--
-- SECURITY: this is the [Mql_admin] counterpart of select_mql_page.sql. Each
-- CTE below shadows a real table with a copy filtered to a single GitLab
-- installation ($installation_id). Unlike select_mql_page.sql it does NOT join
-- gitlab_user_installations2 / filter on $user_id, so it returns data for ANY
