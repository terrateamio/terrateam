...
--
create or replace function trigger_update_gitlab_dirspace_locks_pr()
returns trigger as $$
declare
    result_row record;
begin
    for result_row in
        select * from  update_gitlab_pull_request_dirspace_locks(NEW.repository, NEW.pull_number)
    loop
...
--
create trigger gitlab_pull_requests_dirspace_locks_trigger
    after insert or update on gitlab_pull_requests
    for each ROW
    execute function trigger_update_gitlab_dirspace_locks_pr();

--
-- Now our function for wrapping updates when a work manifest is changed.
--
create or replace function trigger_update_gitlab_dirspace_locks_wm()
returns trigger as $$
declare
    pr_mapping record;
    result_row record;
begin
    if NEW.pull_request is not null then
        ...
        into pr_mapping
        from gitlab_pull_requests_map
        where core_id = NEW.pull_request;
        if FOUND then
            for result_row in
                select * from  update_gitlab_pull_request_dirspace_locks(pr_mapping.repository_id, pr_mapping.pull_number)
            loop
                RAISE NOTICE 'DIRSPACE : LOCK_CHANGE : %', result_row;
            end loop;
...
--
create trigger gitlab_work_manifests_dirspace_locks_trigger
    after insert or update on work_manifests
    for each ROW
    execute function trigger_update_gitlab_dirspace_locks_wm();


--
-- Wrapper function for when the locks table is changed
--
create or replace function trigger_update_gitlab_dirspace_locks_unlock()
returns trigger as $$
declare
    pr_mapping record;
    result_row record;
...
--
create trigger gitlab_unlock_dirspace_locks_trigger
    after insert on pull_request_unlocks
    for each ROW
    execute function trigger_update_gitlab_dirspace_locks_unlock();
...
--
create table if not exists gitlab_dirspace_locking_migration (
    repository bigint,
    pull_number bigint,
    primary key (repository, pull_number)
);

insert into gitlab_dirspace_locking_migration (repository, pull_number)
select repository_id, pull_number from gitlab_pull_requests_map;
