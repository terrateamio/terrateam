module Make = functor (Abb : Abb_intf.S) -> struct
  ...
  let close_write_cb () =
    ...
    Abb.File.open_file
      ~flags:Abb_intf.File.Flag.([Read_only])
      "/tmp/foo.txt"
    >>= function
    | Ok file ->
  ...
  let file_io_test =
    Oth_abb.test
      ~desc:"Simple file I/O test"
      ~name:"File I/O"
      (fun () ->
         let open Abb.Future.Infix_monad in
         Abb.File.open_file
           ~flags:Abb_intf.File.Flag.([Write_only; Create 0o666])
           "/tmp/foo.txt"
         >>= function
         | Ok file -> begin
             ...
             Abb.File.write
               file
               Abb_intf.Write_buf.([ { buf = buf; pos = 0; len = Bytes.length buf } ])
             >>= function
             | Ok _ ->
                 Abb.File.close file
                 >>= fun _ ->
