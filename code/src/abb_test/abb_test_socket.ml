...
module Make = functor (Abb : Abb_intf.S) -> struct
  ...
  let connect_server port =
    ...
    Abb.Socket.getaddrinfo
      ~hints:Abb_intf.Socket.([ Addrinfo_hints.Socket_type Socket_type.Stream
                              ; Addrinfo_hints.Family Domain.Inet4
                              ])
    ...
    | Ok rs -> begin
        ...
        | Ok () ->
          ...
          Abb.Socket.Tcp.send
            tcp
            ~bufs:Abb_intf.Write_buf.([{buf; pos; len}])
          >>= begin function
            | Ok n -> begin
        ...
        | Error _ ->
          failwith "connect error"
      end
    | Error _ ->
      failwith "getaddrinfo error"
  ...
  let start_server cb =
    ...
    let addr =
      Abb_intf.Socket.Sockaddr.(Inet { addr = Unix.inet_addr_any; port = 0 })
    in
    let tcp = CCResult.get_exn (Abb.Socket.Tcp.create ~domain:Abb_intf.Socket.Domain.Inet4) in
    ...
    in
    let both _ _ = () in
    both <$> cb_fut <*> accept_fut
    >>| fun () -> ()

  let socket_test =
    Oth_abb.test
      ~desc:"Simple socket server test"
      ~name:"Socket server test"
      (fun () -> start_server connect_server)
