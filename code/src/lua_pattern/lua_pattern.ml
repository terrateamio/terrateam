module Capture = struct
  ...
  let to_string t = String.sub t.str t.c_s (t.c_e - t.c_s)
end
...
module S = struct
  ...
  let create ~start ~src ~pat =
    { start; src; src_len = String.length src; pat; pat_len = String.length pat; captures = [] }
end
...
let rec validate_pattern captures p s len =
  if p >= len && captures = 0 then
    ...
    match s.[p] with
      | '%' when p + 1 >= len -> None
      | '%' when s.[p + 1] = 'b' && p + 3 < len -> validate_pattern captures (p + 4) s len
      | '%' when s.[p + 1] = 'b' -> None
      | '%' -> validate_pattern captures (p + 2) s len
      | '[' -> validate_bracket captures (p + 1) s len
      | '(' -> validate_pattern (captures + 1) (p + 1) s len
      | ')' when captures = 0 -> None
      | ')' -> validate_pattern (captures - 1) (p + 1) s len
      | _ -> validate_pattern captures (p + 1) s len
...
and validate_bracket captures p s len =
  if p >= len then
    ...
    match s.[p] with
      | '%' when p + 1 < len -> validate_bracket captures (p + 2) s len
      | ']' -> validate_pattern captures (p + 1) s len
      | _ -> validate_bracket captures (p + 1) s len
 ...
 *)
let between_range c pat_prev pat_next = c >= pat_prev && c <= pat_next

let prev_pat_c s pat_p = Pos.pat_c s (Pos.inc ~n:(-1) pat_p)

let next_pat_c s pat_p = Pos.pat_c s (Pos.inc pat_p)

let rec mtch_bracket_pat c s pat_p =
  match Pos.pat_c s pat_p with
    | ']' -> false
    | '%' when Pos.pat_c s (Pos.inc pat_p) = c -> true
    ...
    | '-' -> mtch_bracket_pat c s (Pos.inc ~n:2 pat_p)
    | p when c = p -> true
    | _ -> mtch_bracket_pat c s (Pos.inc pat_p)
...
let src_and_pat_consumed s src_p pat_p = Pos.is_pat_e s pat_p && Pos.is_src_e s src_p

let create_match s src_p =
  Match.{ range = (s.S.start, Pos.to_int src_p); src = s.S.src; captures = List.rev s.S.captures }

let is_balance_mtch s pat_p = Pos.pat_c s pat_p = '%' && Pos.pat_c s (Pos.inc pat_p) = 'b'
...
let rec do_mtch src_p pat_p = function
  | s when Pos.is_pat_e s pat_p -> Some (create_match s src_p)
  | s when Pos.pat_c s pat_p = '(' ->
      let pat_p = Pos.inc pat_p in
      let capture = Capture.{ c_s = Pos.to_int src_p; c_e = Pos.to_int src_p; str = s.S.src } in
      let s = { s with S.captures = capture :: s.S.captures } in
      if Pos.pat_c s pat_p = ')' then
        do_mtch src_p (Pos.inc pat_p) s
      else
        do_mtch src_p pat_p s
  | { S.captures = c :: cs; _ } as s when Pos.pat_c s pat_p = ')' ->
  ...
  | s when is_balance_mtch s pat_p -> None
  ...
  | s ->
      ...
      if not (Pos.is_pat_e s class_e) then
        match (mtch_single src_p pat_p s, Pos.pat_c s class_e) with
          | (false, '*') | (false, '-') | (false, '?') -> do_mtch src_p (Pos.inc class_e) s
          | (false, _) -> None
          | (true, '+') -> max_expand (Pos.inc src_p) pat_p s (Pos.inc class_e)
          | (true, '*') -> max_expand src_p pat_p s (Pos.inc class_e)
          | (true, '-') -> min_expand src_p pat_p s (Pos.inc class_e)
          | (true, '?') -> do_mtch (Pos.inc src_p) (Pos.inc class_e) s
          | (true, _) -> do_mtch (Pos.inc src_p) class_e s
...
and reduce_furthest_mtch_single src_p pat_p s src_p_e =
  if src_p = src_p_e then
    ...
    match do_mtch src_p_e pat_p s with
      | Some ms -> Some ms
      | None    -> reduce_furthest_mtch_single src_p pat_p s (Pos.inc ~n:(-1) src_p_e)

and min_expand src_p pat_p s class_e =
  match do_mtch src_p class_e s with
    | Some ms -> Some ms
    | None when mtch_single src_p pat_p s -> min_expand (Pos.inc src_p) pat_p s class_e
    | None -> None

and mtch_balance depth src_p pat_p = function
  | s when Pos.is_src_e s src_p -> None
  | s when Pos.src_c s src_p = Pos.pat_c s pat_p -> mtch_balance (depth + 1) (Pos.inc src_p) pat_p s
  ...
  | s -> mtch_balance depth (Pos.inc src_p) pat_p s
...
let rec mtch ?(start = 0) str t =
  ...
  match S.create ~start ~src:str ~pat:t with
    | s when Pos.is_src_e s src_p || Pos.is_pat_e s pat_p -> None
    | s when Pos.pat_c s pat_p = '^' -> do_mtch src_p (Pos.inc pat_p) s
    | s -> (
