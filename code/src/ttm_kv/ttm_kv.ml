...
module Cli = struct
  ...
  let data =
    let doc = "Data, in JSON format" in
    C.Arg.(required & pos ~rev:true 0 (some string) None & info [] ~doc ~docv:"DATA")
...
module Set = struct
  let run api_base vcs installation draft idx output key data () =
    let f () =
      let open Abbs_future_combinators.Infix_result_monad in
      try
...
module Cas = struct
  let run api_base vcs installation draft idx output version key data () =
    let f () =
      let open Abbs_future_combinators.Infix_result_monad in
      try
        let data = Yojson.Safe.from_string data in
