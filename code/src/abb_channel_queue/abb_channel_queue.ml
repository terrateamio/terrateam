module Make (Fut : Abb_intf.Future.S) = struct
  module T = struct
    type 'a t = { queue : ('a * unit Fut.Promise.t) Queue.t
                ; fast_count : int
                ; mutable closed : bool
    ...
    let create ?(fast_count = 0) () =
      ...
      ; fast_send_promise = fast_send_promise
      }
    ...
    let send t msg =
      ...
      if t.closed then
        ...
        Fut.Promise.set recv_promise ()
        >>= fun () ->
        Fut.Promise.future promise
        >>| fun v ->
        `Ok v
      end
      else begin
    ...
    let close_with_abort t =
      ...
      let rec abort_until_empty () =
        if not (Queue.is_empty t.queue) then
          ...
          Fut.abort (Fut.Promise.future promise)
          >>= fun () ->
          abort_until_empty ()
        else
          Fut.return ()
  ...
  module Channel (Q : S) (Msg : Msg)
    : (Abb_channel_intf.Make(Fut).S with type t = Msg.t Q.t and type msg = Msg.t) =
  struct
    type msg = Msg.t
    type t = msg Q.t
  ...
  let to_abb_channel (type m) (t : m T.t) =
    let module Msg = (struct type t = m end : Msg with type t = m) in
