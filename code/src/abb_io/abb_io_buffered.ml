...
module Make (Fut : Abb_intf.Future.S) = struct
  ...
  module View = struct
    type t = { read : (buf:bytes ->
                       pos:int ->
                       len:int ->
                       (int, read_err) result Fut.t)
             ; write : (bufs:(Abb_intf.Write_buf.t list) ->
                        (int, write_err) result Fut.t)
             ; close : (unit -> (unit, close_err) result Fut.t)
             }
  end

  module Bytes_io = struct
    type t = { mutable r_buf : bytes
             ; mutable r_pos : int
             ; w_buf : Buffer.t
             }
    ...
    let create b =
      ...
      let rec read ~buf ~pos ~len =
        if t.r_pos = Bytes.length t.r_buf && Buffer.length t.w_buf > 0 then begin
  ...
  let rec read' t ~buf ~pos ~len =
    assert (pos >= 0);
    assert (len > 0);
    if t.length = 0 || t.pos >= t.length then begin
      let open Fut_comb.Infix_result_monad in
      fill_buffer t
      >>= function
      | 0 ->
  ...
  let read_line_bytes t =
    ...
    | () when Buffer.length b = 0 ->
      Fut.return (Error (`Unexpected End_of_file))
    | () ->
  ...
  let rec write t ~bufs =
    let open Fut_comb.Infix_result_monad in
    if t.length = Bytes.length t.buf then begin
      flushed t
      >>= fun () ->
      write t ~bufs
    end
  ...
  let close_writer t =
    (close_writer' t : (unit, [ close_err | write_err ]) result Fut.t :> (unit, [> close_err | write_err ]) result Fut.t)
end
...
module Of (Abb : Abb_intf.S) = struct
  ...
  let of_file ?(size = 1024) file =
    ...
    let close () =
      ...
      | Error _ -> Error `E_io
    in
    let cb = T.View.({ read; write; close })
    in
    T.of_view ~size cb
