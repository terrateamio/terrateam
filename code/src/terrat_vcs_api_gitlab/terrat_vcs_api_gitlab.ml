...
let fetch_pull_request ~request_id account client repo merge_request_iid =
  let run =
    ...
    let checks = state = "merged" || CCOption.get_or ~default:false mergeable in
    let state =
      match (state, detailed_merge_status, merge_commit_sha, merged_at) with
      | "opened", Some "mergeable", _, _ -> Terrat_pull_request.State.(Open Open_status.Mergeable)
      | "opened", _, _, _ -> Terrat_pull_request.State.(Open Open_status.Merge_conflict)
