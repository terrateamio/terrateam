...
module Make (Abb : Abb_intf.S with type Native.t = Unix.file_descr) : sig
  ...
  (** Convert a TCP socket, with the provided TLS config, and the servername
      into a TLS buffered I/O. *)
  val client_tcp :
  ...
  (** Convert a client socket which connected on the socket that corresponds to
      the Tls instance of the server into a TLS buffered I/O. *)
  val server_tcp :
