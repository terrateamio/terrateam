module Dirspaces =
  struct
    type t = Terrat_api_components_work_manifest_dirspace.t list[@@deriving
                                                                  ((yojson
                                                                    {
                                                                    strict =
                                                                    false;
    ...
    let of_yojson =
      Json_schema.one_of
        (let open CCResult in
           [(fun v ->
               map (fun v -> Kind_drift v)
                 (Terrat_api_components_kind_drift.of_yojson v));
           (fun v ->
              map (fun v -> Kind_index v)
                (Terrat_api_components_kind_index.of_yojson v));
           (fun v ->
              map (fun v -> Kind_pull_request v)
                (Terrat_api_components_kind_pull_request.of_yojson v))])
    let to_yojson =
      function
      | Kind_drift v -> Terrat_api_components_kind_drift.to_yojson v
      | Kind_index v -> Terrat_api_components_kind_index.to_yojson v
      | Kind_pull_request v ->
          Terrat_api_components_kind_pull_request.to_yojson v
  end
type t =
  {
  base_branch: string ;
