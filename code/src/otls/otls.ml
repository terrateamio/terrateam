...
type err = [ `Error ]
type io_err = [ err | `Want_pollin | `Want_pollout ]
...
module Tls_config = struct
  ...
  let verify t =
    Otls_c_bindings.Tls_config.tls_config_verify t

  let insecure_noverifycert t =
    Otls_c_bindings.Tls_config.tls_config_insecure_noverifycert t

  let insecure_noverifyname t =
    Otls_c_bindings.Tls_config.tls_config_insecure_noverifyname t

  let insecure_noverifytime t =
    Otls_c_bindings.Tls_config.tls_config_insecure_noverifytime t

  let set_protocols t protos =
    let bitmask =
      ListLabels.fold_left
        ~f:(fun bm v -> bm lor (Tls_protocols.to_int v))
        ~init:0
        protos
    in
    let ret =
      Otls_c_bindings.Tls_config.tls_config_set_protocols
        t
        (Unsigned.UInt32.of_int bitmask)
    in
    assert (ret = 0);
    ()
  ...
  let set_ciphers t ciphers =
    let ret =
      Otls_c_bindings.Tls_config.tls_config_set_ciphers
        t
        (Tls_ciphers.to_string ciphers)
    in
    match ret with
      | -1 -> Error `Error
      | _ -> Ok ()
...
module Tls = struct
  ...
  let destroy t = Otls_c_bindings.Tls.tls_free t
  ...
  let read t ~pos ~len bytes =
    ...
    match Otls_c_bindings.Tls.tls_read t buf (Unsigned.Size_t.of_int len) with
      | n when n = PosixTypes.Ssize.minus_one ->
        Error `Error
      | n when n = Stubs.tls_want_pollin ->
        Error `Want_pollin
      | n when n = Stubs.tls_want_pollout ->
        Error `Want_pollout
      | n ->
        Ok (PosixTypes.Ssize.to_int n)
