...
module Tls_config = struct
  ...
  let tls_config_error =
    F.foreign
      "tls_config_error"
      C.(t @-> returning string)

  let tls_config_set_ca_file =
    F.foreign
      "tls_config_set_ca_file"
      C.(t @-> string @-> returning int)
  ...
  let tls_config_set_protocols =
    F.foreign
      ...
      C.(t @-> uint32_t @-> returning int)

  let tls_config_set_ciphers =
    F.foreign
...
module Tls = struct
  ...
  let tls_connect_socket =
    F.foreign
      "tls_connect_socket"
      C.(t @-> int @-> string @-> returning int)

  let tls_accept_socket =
    F.foreign
      "tls_accept_socket"
      C.(t @-> ptr t @-> int @-> returning int)

  let tls_read =
    F.foreign
      "tls_read"
      C.(t @-> ocaml_bytes @-> size_t @-> returning P.ssize_t)
  ...
  let tls_peer_cert_subject =
    F.foreign
      "tls_peer_cert_subject"
      C.(t @-> returning string)

  let tls_peer_cert_issuer =
    F.foreign
      "tls_peer_cert_issuer"
      C.(t @-> returning string)

  let tls_peer_cert_hash =
    F.foreign
      "tls_peer_cert_hash"
      C.(t @-> returning string)
  ...
  let tls_peer_cert_notbefore =
    F.foreign
      ...
      C.(t @-> returning PosixTypes.time_t)
...
let tls_configure =
  F.foreign
    "tls_configure"
    C.(Tls.t @-> Tls_config.t @-> returning int)
