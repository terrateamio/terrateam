  ...
  let extract idx s =
    if s.[idx] = '/' then
      match CCString.index_from_opt s (idx + 1) '/' with
        | Some eidx ->
          Some (eidx, CCString.sub s (idx + 1) (eidx - idx - 1))
        | None ->
          Some (String.length s, CCString.sub s (idx + 1) (String.length s - idx - 1))
    else
      None
    ...
    match t with
      ...
      | Path_var (t, v) ->
        ...
        if idx < String.length path then begin
          v idx path
          >>= fun (idx, value) ->
          Some (idx, Witness.Var (wit, value))
        end
        else
          None
      | Query_var (t, (n, v)) ->
