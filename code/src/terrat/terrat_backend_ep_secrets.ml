...
let fetch_secrets storage installation_id =
  Pgsql_pool.with_conn storage ~f:(fun db ->
      Pgsql_io.Prepared_stmt.fetch
        ...
        ~f:(fun name encrypted_value session_key nonce is_file modified_by modified_time ->
          Terrat_data_backend.Response.Secret.
            { name; encrypted_value; session_key; nonce; is_file; modified_by; modified_time })
        installation_id)
